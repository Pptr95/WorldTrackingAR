...
class ViewController: UIViewController {
    ...
    @IBAction func add(_ sender: UIButton) {
        /*let node = SCNNode(geometry: SCNCylinder(radius: 0.1, height: 0.2))
        node.geometry?.firstMaterial?.diffuse.contents = UIColor.red
        node.position = SCNVector3(0.0, 0.0, -0.3)
        node.eulerAngles = SCNVector3(-90.degreesToRadiants, 0, 0)
        
        let pyramidNode = SCNNode(geometry: SCNPyramid(width: 0.1, height: 0.1, length: 0.1))
        pyramidNode.geometry?.firstMaterial?.diffuse.contents = UIColor.green
        pyramidNode.position = SCNVector3(0.0, 0.0, -0.3)
        ...
        node.eulerAngles = SCNVector3(Float(180.degreesToRadiants), 0, 0)
        self.sceneView.scene.rootNode.addChildNode(node)
